// Ported from t3code apps/marketing/src/lib/homeMotion.ts.
// Runs hero motion (chip drift, parallax) only while the hero is visible,
// pauses when the tab is hidden, and disables movement for reduced-motion users.
// PingStats has no endorsement tracks or terminal caret, so those are optional.
